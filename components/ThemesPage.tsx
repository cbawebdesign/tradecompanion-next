...
// /api/SymbolsByGroup). Live performance %s come with the perf engine — for
// now we present name + symbol count (visually weighted). The click-through
// popup is a mini drill-down: data ribbon for the selected symbol + keyboard
// nav + the standard row actions (flag / right-click → watchlist / AHK / copy).
...
const EMOJI: [RegExp, string][] = [
  ...
  [/real estate|reit|housing/i, '🏠'], [/power|grid|utility/i, '🔌'], [/water/i, '💧'],
  [/cloud|software|saas|tech/i, '💻'], [/auto|car/i, '🚗'], [/aero|airline|aviation/i, '✈️'],
  [/food|restaurant|beverage|agri|farm/i, '🍔'], [/gaming|game|casino|gambl/i, '🎮'], [/media|stream|entertain/i, '🎬'],
  [/industrial|manufactur/i, '🏭'],
]
...
export function ThemesPage() {
  ...
  const favoriteThemes = useStore(s => s.favoriteThemes)
  ...
  const atFavCap = favoriteThemes.length >= 10
  ...
  const rows = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      const fa = favoriteThemes.includes(favKey(a)) ? 1 : 0
      const fb = favoriteThemes.includes(favKey(b)) ? 1 : 0
      if (fa !== fb) return fb - fa
      return b.count - a.count
    })
  }, [groups, kind, query, favoriteThemes])

  const maxCount = useMemo(() => rows.reduce((m, g) => Math.max(m, g.count), 1), [rows])

  const openPopup = useCallback((g: Group) => {
    setPopup(g); setPopLoading(true); setPopSymbols([]); setSelIndex(0)
  ...
  useEffect(() => {
    if (selSymbol) setSelectedSymbol(selSymbol)
  }, [selSymbol, setSelectedSymbol])

  // Popup keyboard nav — ↑/↓ cycle symbols (ribbon follows), space fires AHK,
  // Esc closes. Mirrors the watchlist behavior Justin asked for.
  useEffect(() => {
    if (!popup) return
    const onKey = (e: KeyboardEvent) => {
      const tag = (e.target as HTMLElement)?.tagName
      if (tag === 'INPUT' || tag === 'TEXTAREA') return
      if (e.key === 'ArrowDown') { e.preventDefault(); setSelIndex(i => Math.min(popSymbols.length - 1, i + 1)) }
      else if (e.key === 'ArrowUp') { e.preventDefault(); setSelIndex(i => Math.max(0, i - 1)) }
      else if (e.key === ' ') {
        e.preventDefault()
        const s = popSymbols[selIndex]
        if (s && config.ahkEnabled && config.ahkUrl) fireAhk(s, config.ahkUrl)
      } else if (e.key === 'Escape') { setPopup(null); setCtxMenu(null) }
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [popup, popSymbols, selIndex, config.ahkEnabled, config.ahkUrl])
  ...
  const onSymbolClick = (sym: string, idx: number) => {
    setSelIndex(idx)
  ...
  return (
    <div className="flex flex-col h-full">
          ...
          <div className="flex rounded-lg overflow-hidden" style={{ border: '1px solid var(--border-glass)' }}>
            {(['theme', 'sector'] as Kind[]).map(k => (
              <button key={k} onClick={() => setKind(k)}
                ...
                style={kind === k ? { background: 'var(--accent-primary)', color: '#fff' } : { background: 'transparent', color: 'var(--text-secondary)' }}>
                {k}s
              </button>
            ))}
          </div>
          {atFavCap && <span className="text-[11px]" style={{ color: 'var(--text-muted)' }}>★ max 10</span>}
        </div>
        <input value={query} onChange={e => setQuery(e.target.value)} placeholder={`Filter ${kind}s…`}
      ...
      <div className="flex-1 overflow-auto px-2 py-2">
        ...
        {!loading && error && <div className="p-8 text-center text-sm" style={{ color: 'var(--text-muted)' }}>Couldn&apos;t load ({error}).</div>}
        ...
        {!loading && !error && rows.length > 0 && (
          <div className="flex flex-col gap-1">
            {rows.map(g => {
              const isFav = favoriteThemes.includes(favKey(g))
              ...
              return (
                <button key={favKey(g)} onClick={() => openPopup(g)}
                  ...
                  <span onClick={(e) => { e.stopPropagation(); toggleFavoriteTheme(favKey(g)) }}
                    className={clsx('text-base leading-none', isFav ? 'text-yellow-400' : 'text-gray-600 hover:text-gray-400')}
                    title={!isFav && atFavCap ? 'Favorite limit reached (10)' : 'Favorite'}>
                    {isFav ? '★' : '☆'}
                  </span>
      ...
      {/* Component popup — mini drill-down with data ribbon + keyboard nav */}
      {popup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.65)' }}
          onClick={() => { setPopup(null); setCtxMenu(null) }}>
          <div className="glass-panel rounded-lg w-full max-w-3xl max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-2.5 border-b flex-shrink-0" style={{ borderColor: 'var(--border-glass)' }}>
              <div className="text-sm flex items-center gap-2">
                ...
                <span className="text-xs" style={{ color: 'var(--text-muted)' }}>{popSymbols.length} symbols · ↑/↓ to cycle</span>
              </div>
            ...
            <div className="flex-1 overflow-auto">
              ...
              {!popLoading && popSymbols.length > 0 && (
                <table className="w-full text-sm">
                  <tbody>
                    {popSymbols.map((sym, idx) => {
                      const isFlagged = flaggedSymbols.has(sym)
                      const v = quotes[sym]?.changePercent
                      const isSel = idx === selIndex
                      return (
                        <tr key={sym}
                          className={clsx('cursor-pointer border-b hover:bg-gray-700/30', isSel && 'bg-blue-900/40')}
                          style={{ borderColor: 'var(--border-glass)' }}
                          onClick={() => onSymbolClick(sym, idx)}
                          onContextMenu={(e) => { e.preventDefault(); setSelIndex(idx); setCtxMenu({ x: e.clientX, y: e.clientY, symbol: sym }) }}>
